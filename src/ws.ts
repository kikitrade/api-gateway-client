...
class WS {
    ...
    send(method: string, path: string, webSocketApiType?: WebSocketApiType, body?: string | FormData): void {
        ...
        if (this.config.authType === 'appCode') {
            const msg = {
                ...
                headers: Object.assign({
                    ...
                    'date': new Date().toUTCString(),
                    'x-ca-timestamp': new Date().getTime().toString(),
                    'Authorization': `APPCODE ${this.config.appCode as string}`,
                    'isBase64': 0,
                    'ca_version': '1',
                }, webSocketApiType ? {'x-ca-websocket_api_type': webSocketApiType} : {}),
                path: path,
        ...
        const headers = util.createSignedRequestHeaders('POST', new URL(path, this.config.url).toString(), {},
            (body && body instanceof FormData) ? Content_Type_Form_Data : Content_Type_Json_Data, Content_Type_Json_Data, webSocketApiType, body)
        const msg = {
            method: 'POST',
            host: this.host,
            headers: headers,
    ...
    private unregMsg(host: string, unregisterPath: string = '/unregister', body?: string | FormData) {
        ...
        const headers = util.createSignedRequestHeaders('POST', new URL(unregisterPath, this.config.url).toString(), {},
            (body && body instanceof FormData) ? Content_Type_Form_Data : Content_Type_Json_Data, Content_Type_Json_Data, 'UNREGISTER', body)
        const msg = {
            method: 'POST',
            host: host,
            headers: headers,
